...
game.Button = me.GUI_Object.extend({
    ...
    init: function (x, y, settings) {
        ...
        if (settings.font) {
            this.font = new me.BitmapFont(settings.font, { x: 24 });
        }
 ...
 * @extends game.Button
 */
game.StateButton = game.Button.extend({
    /**
     * Initialise the object
     * @param  {integer} x
     * @param  {integer} y
    ...
    init: function (x, y, settings) {
        this.parent(x, y, settings);
        this.state = settings.state.toUpperCase();
        if (!settings.state) {
            throw "Requires state!";
        }
    },
     ...
     */
    onClick: function () {
        var state = eval("me.state." + this.state);
...
game.HUDText = me.HUD_Item.extend({
     ...
     * @return {void}
     */
    init: function (x, y, fontName, alignment, scale, xVal) {
        this.parent(x, y);
        if (fontName) {
            if (!xVal) {
                throw "Missing xVal argument";
            }
            this.font = new me.BitmapFont(fontName, { x: xVal });
        } else {
            this.font = new me.BitmapFont("atascii", { x: 24 });
        }
        if (alignment && scale) {
...
game.LifeObject = game.HUDText.extend({
     ...
     */
    init: function (x, y) {
        this.parent(x, y, "lifeHeart", "left", 0.7, 99);
    },
...
game._HUDText = game.HUDText.extend({
     ...
     * @return {void}
     */
    init: function (x, y, texts, fontName, alignment, scale, xVal) {
        
        this.parent(x, y, fontName, alignment, scale, xVal);
    ...
    draw: function (context, x, y) {
        ...
        var text = this.text1 + val + this.text2;
    ...
    */
    init: function (x, y, settings) {
        ...
        if (settings.alignment) {
            font = settings.alignment;
        }
        
        this.HUDWidget = new game._HUDText(x, y, text, font, alignment, 1.6, 24);
        if (!settings.tag) {
            this.tag = this.GUID;
        } else {
...
game.BenchMarkTest = game.HUDItem.extend({
   /**
    * Initialise object
    * @param  {integer} x
    * @param  {integer} y
    ...
    */
    ...
    update: function () {
        //this.value = eval("this.objectTracked." + this.valueFrom);
        me.game.HUD.setItemValue(this.tag, me.sys.fps);
    },
});
...
game.MiniMapHUD = me.HUD_Item.extend({
    ...
    init: function (rect, entitiesTracked, focusedEntity) {
        ...
        if (this.entities.length == 0) {
            throw "Have you placed this in a high enough layer in Tiled? Are there any entities on stage?";
        }
    ...
    drawVisionCone: function (context, x, y, a, colour) {
        var grd = context.createRadialGradient(x, y, 0, x, y, 30);
        grd.addColorStop(0, "rgba(" + colour + ",0.6)");
        grd.addColorStop(1, "rgba(" + colour + ",0)");
        context.beginPath();
        context.arc(x, y, 30, a - 0.7, a + 0.7, false);
        context.lineTo(x, y);
        context.closePath();
