 ...
 * @return {void}
 */
Helper.pointWithinAngle = function (direction, dRange, point1, point2) {
    var range = dRange / 2;
...
Light.GetBox = function (x, y) {
    ...
    if (x > collisionLayer.length - 1 || y > collisionLayer[0].length - 1) return null;
...
Visible.TwoPointsCanSee = function (point1, point2) {
    ...
    for (var i = 0; i < obj.length; i++) {
        ...
        for (var j = 0; j < segments.length; j++) {
            var intersect = Visible.CheckIntersection(segments[j].point1, segments[j].point2, point1, point2);
            if (intersect) {
                return false;
            }
        }
