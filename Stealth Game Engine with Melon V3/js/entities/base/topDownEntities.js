...
game.TopDownPlayerEntity = game.MainPlayer.extend({
    ...
    update: function () {
        ...
        var y = ~~this.collisionBox.top + collisionLayer.tileheight;// this.collisionBox.height;// + collisionLayer.tileheight;// * me.sys.scale.y;
...
game.TopDownEnemy = game.TopDownEntity.extend({
    ...
    update: function () {
        ...
        if (me.timer.getTime() >= this.timer + 1000) {
            //this.light.updateLightPos(this.pos.x + this.width / 2, this.pos.y + this.height / 2);
            var canSee = Visible.TwoPointsCanSee(this.pos, game.playerPos);
            
            var withinAlertRadius = Math.pow(game.playerPos.x - this.pos.x, 2) + Math.pow(game.playerPos.y - this.pos.y, 2) < Math.pow(300, 2);
            var withinSuspiciousRadius = Math.pow(game.playerPos.x - this.pos.x, 2) + Math.pow(game.playerPos.y - this.pos.y, 2) < Math.pow(500, 2);

            this.angleOK = Helper.pointWithinAngle(this.faceDirection, Math.PI / 2, this.pos, game.playerPos);
            if (canSee && this.state == this.STATES.NEUTRAL && withinSuspiciousRadius && this.angleOK) {
                this.spottedPlayerPos = new me.Vector2d(game.playerPos.x, game.playerPos.y);
    ...
    draw: function (context) {
        context.save();
        this.light.draw(context);
        if (Visible.TwoPointsCanSee(this.pos, game.playerPos) && this.angleOK) {
            context.beginPath();
            context.moveTo(this.pos.x, this.pos.y);
            context.lineTo(game.playerPos.x, game.playerPos.y);
            context.stroke();
        }
        context.restore();
        this.parent(context);
    ...
    moveWithPath: function () {
        ...
        if (this.state == this.STATES.ALERTED) {
            var completed = this.goToPoint(this.spottedPlayerPos, 100);
            if (completed) {
                me.state.change(me.state.GAMEOVER)
            }
        }
